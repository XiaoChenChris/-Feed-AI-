...
func main() {
	...
	repo := repositories.NewSocialRepository(sqlDB)
	socialWorker := worker.NewSocialWorker(ch, repo, socialQueue)
	videoRepo := repositories.NewVideoRepository(sqlDB)
	likeRepo := repositories.NewLikeRepository(sqlDB)
	commentRepo := repositories.NewCommentRepository(sqlDB)
