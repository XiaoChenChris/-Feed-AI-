...
func NewSocialWorker(ch *amqp.Channel, repo *repositories.SocialRepository, queue string) *SocialWorker {
...
func (w *SocialWorker) process(ctx context.Context, body []byte) error {
	...
	switch evt.Action {
	case "follow":
		err := w.repo.Follow(ctx, &models.Social{
		...
		return err
	case "unfollow":
		return w.repo.Unfollow(ctx, &models.Social{
			FollowerID: evt.FollowerID,
			VloggerID:  evt.VloggerID,
		})
	default:
