...
func NewLikeWorker(ch *amqp.Channel, likes *repositories.LikeRepository, videos *repositories.VideoRepository, queue string) *LikeWorker {
...
func (w *LikeWorker) applyLike(ctx context.Context, userID, videoID uint) error {
	...
	created, err := w.likes.LikeIgnoreDuplicate(ctx, &models.Like{
		VideoID:   videoID,
		AccountID: userID,
		CreatedAt: time.Now(),
	})
