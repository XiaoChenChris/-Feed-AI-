...
func JWTAuth(accountRepo *repositories.AccountRepository, cache *rediscache.Client) gin.HandlerFunc {
...
func SoftJWTAuth(accountRepo *repositories.AccountRepository, cache *rediscache.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		authHeader := c.GetHeader("Authorization")
		if authHeader == "" {
			c.Next()
...
func softCheck(c *gin.Context, claims *auth.Claims, tokenString string, accountRepo *repositories.AccountRepository, cache *rediscache.Client) {
...
func check(c *gin.Context, claims *auth.Claims, tokenString string, accountRepo *repositories.AccountRepository, cache *rediscache.Client) {
	key := cache.Key("account:%d", claims.AccountID)
