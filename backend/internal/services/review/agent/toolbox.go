...
// AITranscriber is an interface for audio transcription to avoid import cycles.
type AITranscriber interface {
	TranscribeAudio(mp3Path string) (string, error)
	ExtractAudio(videoPath, mp3Path string) error
}
