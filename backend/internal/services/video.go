...
func NewVideoService(repo *repositories.VideoRepository, cache *rediscache.Client, popularityMQ *rabbitmq.PopularityMQ) *VideoService {
...
// GetReviewConfig returns the review config
...
func (vs *VideoService) Publish(ctx context.Context, video *models.Video) error {
	if video == nil {
	...
	err := vs.repo.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		tags := models.ExtractTags(video.Title + " " + video.Description)
		for _, tagName := range tags {
			var tag models.Tag
			tx.Where("name = ?", tagName).FirstOrCreate(&tag, models.Tag{Name: tagName})
			tx.Create(&models.VideoTag{VideoID: video.ID, TagID: tag.ID})
		}
		return nil
	})
	if err != nil {
...
func (vs *VideoService) ReviewAndPublishVideo(v *models.Video) {
	...
	if finalStatus == "approved" {
		_ = vs.repo.CreateMsg(context.Background(), &models.OutboxMsg{
			VideoID:    v.ID,
			EventType:  "video_published",
			CreateTime: time.Now(),
			Status:     "pending",
...
func (vs *VideoService) applyReviewResultWithAgent(videoID uint, status string, result *review.ReviewResult, trace *agent.AgentTrace, phase0, phase1 map[string]interface{}) {
	...
	vs.repo.DB.Model(&models.Video{}).Where("id = ?", videoID).Updates(updates)
}
...
func (vs *VideoService) ListByAuthorID(ctx context.Context, authorID uint) ([]models.Video, error) {
...
func (vs *VideoService) GetDetail(ctx context.Context, id uint, viewerID uint) (*models.Video, error) {
...
func (vs *VideoService) getDetailInternal(ctx context.Context, id uint) (*models.Video, error) {
	cacheKey := vs.cache.Key("video:detail:id=%d", id)

	getCached := func() (*models.Video, bool) {
	...
	setCached := func(video *models.Video) {
