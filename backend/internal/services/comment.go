...
func NewCommentService(repo *repositories.CommentRepository, videoRepo *repositories.VideoRepository, cache *rediscache.Client, commentMQ *rabbitmq.CommentMQ, popularityMQ *rabbitmq.PopularityMQ) *CommentService {
...
func (s *CommentService) Publish(ctx context.Context, comment *models.Comment) error {
	if comment == nil {
	...
	if !mysqlEnqueued {
		if err := s.repo.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			if err := tx.Select("id").First(&models.Video{}, comment.VideoID).Error; err != nil {
			...
			return tx.Model(&models.Video{}).Where("id = ?", comment.VideoID).
	...
	if reviewEnabled {
		...
		go func() {
			...
			if s.reviewService.Classify(result) == "rejected" {
				s.repo.DB.Model(&models.Comment{}).Where("id = ?", commentID).Updates(map[string]interface{}{
...
func (s *CommentService) GetAll(ctx context.Context, videoID uint) ([]models.Comment, error) {
